...
# 2015-12-06
#
# A surprisingly slow algorithm. Faster with numpy?
#

DAY6_INSTRUCTIONS='''turn off 660,55 through 986,197
...
class ChristmasLightsGrid(object):
    
    def __init__(self, width=1000, height=1000, mode=MODE_TOGGLE):
        
        global MODE_TOGGLE, MODE_BRIGHTNESS
        ...
        if self.mode == MODE_TOGGLE:
            value = False
        elif self.mode == MODE_BRIGHTNESS:
            value = 0
        else:
            raise RuntimeError("Unknown mode")
        
        # initialize the grid to all off
    ...
    def __call__(self, instructionString):
        #
        # turn on 0,0 through 999,999
        #
        locationFinder = re.compile('\\d*,\\d*')
        locations = locationFinder.findall(instructionString)
        if len(locations) == 2:
            start, end = locations[0].split(','), locations[1].split(',')
        
            if instructionString.startswith('turn on'):
                self.turnOn(start, end)
            elif instructionString.startswith('turn off'):
                self.turnOff(start, end)
            elif instructionString.startswith('toggle'):
                self.toggle(start, end)
        else:
            raise RuntimeError("Didn't find two lcoations")
            
    def turnOn(self, startLocation, endLocation):
        global MODE_BRIGHTNESS, MODE_TOGGLE
        if self.mode == MODE_TOGGLE:
            onFunction = lambda x: True
        elif self.mode == MODE_BRIGHTNESS:
            onFunction = lambda x: x + 1
            
        self._doOverRange(startLocation, endLocation, onFunction)
                
    def turnOff(self, startLocation, endLocation):
        global MODE_BRIGHTNESS, MODE_TOGGLE
        if self.mode == MODE_TOGGLE:
            offFunction = lambda x: False
        elif self.mode == MODE_BRIGHTNESS:
            offFunction = lambda x: max(x - 1, 0)
            
        self._doOverRange(startLocation, endLocation, offFunction)
        
    def toggle(self, startLocation, endLocation):
        global MODE_BRIGHTNESS, MODE_TOGGLE
        if self.mode == MODE_TOGGLE:
            toggleFunction = lambda x: not x
        elif self.mode == MODE_BRIGHTNESS:
            toggleFunction = lambda x: x + 2
            
        self._doOverRange(startLocation, endLocation, toggleFunction)
                
    def _doOverRange(self, startLocation, endLocation, actionFunction):
        startX, startY = int(startLocation[0]), int(startLocation[1])
        endX, endY = int(endLocation[0]), int(endLocation[1])
        
        for i in range(startX, endX + 1):
            for j in range(startY, endY + 1):
                self.grid[i][j] = actionFunction(self.grid[i][j])
                
    def countAllTheLights(self):
        totalCount = 0
        
        for i in range(self.width):
            totalCount += self.grid[i].count(True)
            
        return totalCount
    
    def howBrightIsIt(self):
        totalBrightness = 0
        
        for i in range(self.width):
            for j in range(self.height):
                totalBrightness += self.grid[i][j]
                
        return totalBrightness
    
if __name__ == '__main__':
    
    testLights = ChristmasLightsGrid()
    testLights('turn on 0,0 through 999,999')
    print('There are {0:d} lights on in the first case'.format(testLights.countAllTheLights()))

    testLights('toggle 0,0 through 999,0')
    print('There are {0:d} lights on in the second case'.format(testLights.countAllTheLights()))
    
    testLights('turn off 499,499 through 500,500')
    print('There are {0:d} lights on in the third case'.format(testLights.countAllTheLights()))
    
    print('\n')
    
    instructions = DAY6_INSTRUCTIONS.splitlines()
    
    #
    # Run the instructions in toggle mode
    #
    
    christmasLights = ChristmasLightsGrid()
    for instruction in instructions:
        christmasLights(instruction)
        
    print('There are {0:d} lights on following the day 6 instructions in toggle mode.'.format(christmasLights.countAllTheLights()))
    
    #
    # Now, brightness mode
    #
        
    christmasLights = ChristmasLightsGrid(mode=MODE_BRIGHTNESS)
    for instruction in instructions:
        christmasLights(instruction)                
        
    print('The total brightness is {0:d} following the day 6 instructions in brightness mode.'.format(christmasLights.howBrightIsIt()))

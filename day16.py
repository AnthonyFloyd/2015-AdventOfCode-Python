...
day16Sues = '''Sue 1: children: 1, cars: 8, vizslas: 7
...
def parseSues(suesList):
    
    sues = []
    
    for sueLine in suesList:
        sueLineBits = sueLine.split()
        dummy = sueLineBits.pop(0)
        sueId = int(sueLineBits.pop(0)[:-1])
        newSue = Sue(sueId)
        
        while len(sueLineBits) > 0:
            identifier = sueLineBits.pop(0)[:-1]
            count = int(sueLineBits.pop(0).replace(',',''))
            setattr(newSue, identifier, count)
